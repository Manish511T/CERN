...
import UserAlertMap from './UserAlertMap'
...
const AlertBanner = () => {
    ...
    socket.on('sos:accepted', (data) => {
      if (data.openMap) {
        ...
        startTracking({
          ...
          victimLocation: data.victimLocation || (currentAlert?.location
            ? {
  ...
  // User map — full screen static pin
  if (userMapOpen && alert && !alert.isVolunteer) {
    return (
      <UserAlertMap
        alert={alert}
        onClose={() => {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
      <div className="bg-white border-2 border-red-500 rounded-2xl shadow-2xl p-4">
        ...
        {/* Distance indicator for nearby users */}
        {!alert.isVolunteer && alert.location?.coordinates && (
          <p className="text-orange-600 text-xs font-medium mb-2">
            ⚠️ This person is within 500m of your location
          </p>
        )}
        ...
        <div className="flex gap-2 mt-2">
          {alert.isVolunteer ? (
            ...
            // Nearby user buttons — show map with victim pin
            <>
              <button
